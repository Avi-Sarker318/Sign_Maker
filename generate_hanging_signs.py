#!/usr/bin/env python3
"""
Hanging Signs barcode generator (command line).

Reads a list of labels, groups them by the prefix before the "-", and writes
one landscape PDF per prefix into ./output/Hanging Signs PDFS. Each label
becomes one full page (unlike Sticky Signs, which fits two per page).

    a-001, a-002, a-003, c-001, c-004   ->   a.pdf  (3 pages)
                                             c.pdf  (2 pages)
...
Labels can be supplied four ways:
    ...
    python generate_hanging_signs.py                    # LABELS list below
    python generate_hanging_signs.py a-001 a-002        # on the command line
    python generate_hanging_signs.py --file labels.txt  # one per line
                                                        # or prompts if empty
...
matter what order you list them in. Add --update to write into the same run
folder each time (replacing the old PDFs) instead of making a new _2/_3
folder, which is what you want when you're building up one set of signs over
multiple sessions rather than making separate batches.

Prefer clicking a button instead of typing commands? Use sign_maker_gui.py
(or the SignMaker.exe built from it) instead - same logic, a window.
...
from hanging_signs_core import generate_signs
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Hanging Signs barcode PDFs, one per label prefix."
    )
